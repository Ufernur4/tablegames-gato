...
export function Darts({ game: initialGame, userId, onLeave }: DartsProps) {
  ...
  // Submit all 3 darts at once
  const submitRound = useCallback(async (totalPoints: number) => {
  ...
  const onHit = useCallback((hit: { points: number; label: string }, dart: { x: number; y: number }) => {
    ...
    // Accumulate points - don't submit yet
    setRoundPoints(prev => {
      const updated = [...prev, hit.points];
      // After 3rd dart, submit round total
      if (updated.length >= 3) {
  ...
  useEffect(() => { reset(); setRoundPoints([]); }, [game.current_turn, reset]);

  const roundTotal = roundPoints.reduce((a, b) => a + b, 0);
  ...
  const statusText = game.status === 'waiting' ? 'Warte auf Mitspieler…' :
    ...
    isMyTurn ? `Dart ${currentDart + 1}/3 — Wische nach oben ↑` : 'Gegner wirft…';
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        <main className="flex-1 flex flex-col items-center p-4 gap-3 overflow-y-auto">
          ...
          {isMyTurn && roundPoints.length > 0 && (
            <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-primary/10 border border-primary/20">
              <span className="text-xs text-muted-foreground">Runde:</span>
              {roundPoints.map((p, i) => (
                <span key={i} className={`text-sm font-bold px-2 py-0.5 rounded-lg ${
                  p >= 50 ? 'bg-primary/20 text-primary' :
                  p >= 20 ? 'bg-accent/20 text-accent-foreground' :
                  'bg-secondary text-secondary-foreground'
                }`}>{p}</span>
              ))}
              <span className="text-sm font-black text-primary ml-1">= {roundTotal}</span>
            </motion.div>
          )}
          ...
          <AnimatePresence>
            {throwState.isDragging && (
              <motion.div initial={{ opacity: 0, scaleY: 0 }} animate={{ opacity: 1, scaleY: 1 }}
                ...
                <motion.div animate={{ height: `${throwState.power * 100}%` }}
            ...
            <div ref={containerRef}
              ...
              <motion.div
                animate={{
                  ...
                  rotateY: throwState.isDragging ? (throwState.dragCurrent.x - throwState.dragStart.x) * 0.03 : 0,
